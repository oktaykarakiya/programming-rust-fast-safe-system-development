...
fn loopieloop(c:Complex<f64>, limit: usize) -> Option<usize> {
...
fn render(pixels: &mut [u8], bounds: (usize, usize), upper_left: Complex<f64>, lower_right: Complex<f64>){
    ...
    for row in 0..bounds.1 {
        for column in 0..bounds.0 {
            ...
            pixels[row * bounds.0 + column] = match loopieloop(point, 255) {
                None => 0,
...
fn write_imagae(filename: &str, pixels: &[u8], bounds:(usize, usize)) -> Result<(), std::io::Error> {
...
fn main(){
    ...
    if args.len() != 5 {
        eprintln!("Usage: {} FILE PIXELS UPPERLEFT LOWERRIGT", args[0]);
    ...
    let upper_left = parse_complex(&args[3])
        .expect("error parsing UL corner");
    let lower_right = parse_complex(&args[4])
        .expect("error parsing LR corner");

    let mut pixels = vec![0; bounds.0 * bounds.1];

    render(&mut pixels, bounds, upper_left, lower_right);

    write_imagae(&args[1], &pixels, bounds)
